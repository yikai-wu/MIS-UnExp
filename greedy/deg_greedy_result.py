...
def maximum_independent_set_greedy(graph):
    ...
    while len(temp_graph) > 0:
        ...
        node = min(temp_graph.nodes(), key=lambda n: temp_graph.degree(n))
...
def process_graph_file(graph_path, output_folder):
    ...
    independent_set = maximum_independent_set_greedy(graph)
